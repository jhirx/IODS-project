...
##pairs(alc[!names(alc) %in% c("high_use")],col=alc$high_use)

# create a more advanced plot matrix with ggpairs()
#ggpairs(alc, 
#        mapping = aes(col = sex, alpha = 0.3), 
#        lower = list(combo = wrap("facethist", bins = 20)))

##qplot( high_use, internet, data = alc) + geom_smooth(method = "lm")
...
#pairs(m[!names(m) %in% c("high_use")],col=m$high_use)
